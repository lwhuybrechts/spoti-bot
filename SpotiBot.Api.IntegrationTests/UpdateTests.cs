...
namespace SpotiBot.Api.IntegrationTests
{
    public class UpdateTests : IClassFixture<TestHost>
    {
        ...
        public UpdateTests(TestHost testHost)
        {
            ...
            _authService = testHost.GetService<IAuthService>();
        ...
        public async Task Run_LastDownvoteCallback_TrackDeleted()
        {
            ...
            var track = await GetTrack();
            Assert.Equal((int)TrackState.RemovedByDownvotes, track.State);
        }
        ...
        private async Task RemoveTrackFromPlaylist()
        {
            ...
            var spotifyClient = await _authService.GetClient(_testOptions.TestUserId);
